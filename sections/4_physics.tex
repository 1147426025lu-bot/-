...
\begin{equation}
	...
	\label{eq:leg-energy}
\end{equation}
其中 $m_g^{\mathrm{empty}}$ 为含电池空载总质量（按题给三机型参数取值\cite{dji2024m350}），$g_0$ 为重力加速度（取 $g_0$ 而非 $g$，是为避开机型编号 $g$，全文同）。第一项由能量与航程之比得出，量纲已是 kWh；第二项按 $m g_0 h/\eta$ 算出的是焦耳，故除以 $3.6\times10^{6}$ 折算为 kWh 后再相加，两项方可同量纲求和。下降能耗效率取 0，不单独计算下降附加能耗。每一运输架次 $p$ 需满足返航安全余量
\begin{equation}
...
无人驾驶航空器用锂离子电池和电池组的通用规范见国家标准\cite{samr2025uavbattery}。共享电池与中继能源组件作为独立能源资源记录荷电状态（SOC）。任务结束 SOC 为 $s$ 的资源充电至 100\% 所需时间按题给两阶段等效充电模型（快充段占等效完全充电时间的 65\%，慢充段占 35\%，各阶段按 SOC 增量线性折算；这与锂离子电池充电器“恒流—恒压”两阶段策略的工程实践相符\cite{ti2017charger}）：
\begin{equation}
	t_{\mathrm{chg}}(s)=
	\begin{cases}
...
三态占比一律按\textbf{时间积分}统计：以 $\Delta t=1$\,s 离散化轨迹，第 $m$ 个采样点取权重 $w_m=(t_{m+1}-t_{m-1})/2$（用 $m$ 而非 $i$ 作采样点下标，是为避开任务节点编号 $i,j$），再按状态加权累加，故占比是时间占比而非采样点个数占比——轨迹在爬升、巡航、悬停交接各段的采样密度并不相同，按点数统计会系统性偏向采样更密的阶段。判定「中继」时须同时满足两个条件：该悬停站此刻\textbf{确实能连通}这架运输机（逐点计算接入链路的路径损耗与门限），且对应的中继架次此刻已完成建链、尚未结束服务。只检查时刻是否落在某个在役中继的时间窗内是不够的——那会把「站在服务、但几何上够不着」的时段误记为已覆盖，从而虚高中继覆盖率。\label{sec:comm-caliber}

\noindent\textbf{本节链路判据的口径边界}须一并说明：上述判定是\textbf{几何式的二值视线判定}——沿视线连线按 $5$\,m 步进采样 DEM，采样点高出地面 $1$\,mm 即判为无遮挡；判为无遮挡的链路再按自由空间传播损耗叠加固定的地形遮挡附加损耗 $L_{\mathrm{obs}}=10$\,dB，双向取较小门限定其可用性。它\textbf{未包含第一菲涅尔区净空、绕射与多径}：判定只问「连线是否被地形切断」，不问「切断处还剩多少余隙」。因此掩体边缘附近的链路可能被本文判为可用、而在实际传播中并不可用，本文给出的直连可用时段、覆盖率与直连时间占比都应读作\textbf{偏乐观的上界}。本文没有为这一项预留额外的几何裕量——预留裕量会让判定与代码实现脱节，论文里的数就不再对应可复现的结果。要收紧须在链路模型里真正加入菲涅尔半径判据并重算，本文未做；第~\ref{sec:evaluation}~章另有同一条局限的陈述，两处指的是同一件事。
