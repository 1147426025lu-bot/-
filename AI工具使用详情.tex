...
% 四个字段（工具名称／版本或型号／开发机构／版本发布日期）已于下表中填齐，见「记录项目」各行。
% 使用记录须据实增删，不得编造。
...
\vspace{1em}
\noindent\textbf{声明：}本参赛队对上述记录的真实性负责。人工智能工具的参与范围以上表所列为限，
即建模方案的讨论与比较、代码编写与排错、图表生成与排版、文字润色四类辅助工作；
\textbf{模型方程、约束条件与求解算法均由本队独立推导确定}，工具给出的建议仅供参考，
采纳与否由本队逐项判断，全部结果数字由本队脚本运行产生并经独立复核。
