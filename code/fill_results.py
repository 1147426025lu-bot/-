...
SHEETS = [
    ...
    # 出动编号 / 架次内序 / 本访问能耗kWh 是阶段 13（中继站间接续）新增的列：
    # 模板一行按「一次中继架次」给位，而 CSV 一行 = 一次悬停站服务，同一次出动的
    # 多行在出动级列上取同值。三列都是表内复核的旁证（去重键、接续次序、能耗份额），
    # 模板没有对应列，故与悬停站编号一并逐名列明不提交。
    ('Q3_中继架次', 'q3_relay_trips.csv',
     ('悬停站编号', '出动编号', '架次内序', '本访问能耗kWh')),
